...
/*
 ...
 
 {
     ...
     "sentiment_votes_up_percentage": 67.03,
     "sentiment_votes_down_percentage": 32.97,
     "watchlist_portfolio_users": 2295316,
     "market_cap_rank": 1,
     "status_updates": [],
     "last_updated": "2025-12-26T15:24:44.982Z"
 }
...
extension CoinDetailModel {
    func toPresentationModel() -> CoinDetailPresentationModel {
        return CoinDetailPresentationModel(
            id: self.id ?? "",
            symbol: self.symbol ?? "",
            name: self.name ?? "",
            blockTimeInMinutes: self.blockTimeInMinutes ?? 0,
            hashingAlgorithm: self.hashingAlgorithm ?? "",
            categories: self.categories ?? [],
            description: DescriptionDetailPresentationModel(en: self.description?.en ?? ""),
            links: LinksDetailPresentationModel(homepage: self.links?.homepage ?? [], subredditURL: self.links?.subredditURL ?? ""))
    }
}
