...
struct ChartView: View {
    ...
    init(coin:CoinPresentationModel) {
        ...
        print("Price change: if \(priceChange) > 0 then green, else red")
...
extension ChartView {
    ...
    private var chartYAxis : some View {
        VStack {
            ...
            Spacer()
            let price:String = (((maxY + minY) / 2).formattedWithAbbreviations())
            Text(price)
            Spacer()
...
#Preview {
    ChartView(coin: DeveloperPreview.dev.coin.toPresentationModel())
        .frame(width: 300)
}
