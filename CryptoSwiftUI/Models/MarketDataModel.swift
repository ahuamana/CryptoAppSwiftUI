...
struct GlobalMarketDataModel : Decodable {
...
extension MarketDataModel {
    func toPresentationModel() -> MarketPresentationModel {
        return MarketPresentationModel(
            totalMarketCap: totalMarketCap ?? [:],
            totalVolume: totalVolume ?? [:],
            marketCapPercentage: marketCapPercentage ?? [:],
            marketCapChangePercentage24HUsd: marketCapChangePercentage24HUsd ?? 0.0
        )
    }
}
